...
// POST /buyers to the database
  app.post('/buyers', function (req, res) {
    ...
    // if (('id' in req.body) && ('offers' in req.body)) {
      // var id = req.body.id
      // var buyer = JSON.stringify(req.body)
      // client.hset('buyers', id, buyer, function (err, reply) {
      //   if (!err) {
      //     if (reply === 1) {
      //       res.status(201).send({'Success': `Buyer ${id} added successfully`})
      //     } else if (reply === 0) {
      //       res.status(201).send({'Existing Buyer': `Buyer ${id} already exists` })
      //     }
      //   } else {
...
// GET /buyers/:id
  ...
  //       if (data) {
  //           	console.log('matching: ' + data)console.log('matching: ' + data)
  //           	res.status(200).send(data)
  //       } else {
  //           	res.status(200).json({'Error': 'No match'})
  //       }
  //     } else {
...
// GET route
  app.get('/route', function (req, res) {
    ...
    // var queryDevice = req.query.device
    ...
    //       }
    //     }
    //     res.status(302).json({'location': maxLocation})
    //   } else {
    //     res.status(500).send({'Error': 'Internal Server Error'})
    //   }
    // })
